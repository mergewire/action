...
export function extractPricingResources(planJson: unknown): PricingResource[] {
  ...
  for (const change of resourceChanges) {
    ...
    const before = changeData.before as
      | Record<string, unknown>
      | undefined
      | null;
    const after = changeData.after as
      | Record<string, unknown>
      | undefined
      | null;
    const extracted = extractDimensions(type, before, after);

    const pricingResource = buildPricingResource(
      address,
      type,
      action,
      extracted,
...
function determineSingleAction(
  normalizedActions: ResourceAction[],
): ResourceAction {
...
function extractGcpSqlDimensions(
  ...
  const engine = normalizeGcpDatabaseVersion(
    extractString(obj, "database_version"),
  );
  ...
  const availabilityType = normalizeValue(
    extractString(settings, "availability_type"),
  );
...
function extractGcpRedisDimensions(
  ...
  return {
    ...
    highAvailability,
    replicaCount:
      extractNumber(obj, "replica_count") ?? (highAvailability ? 1 : 0),
  };
}
...
function extractAzureManagedDiskDimensions(
  ...
  const normalizedDisk = normalizeAzureManagedDiskSku(
    storageAccountType,
    sizeGiB,
  );
...
function normalizeGcpDatabaseVersion(
...
function normalizeAzureManagedDiskSku(
...
function mapAzureManagedDiskName(
  ...
  const diskSizes =
    AZURE_MANAGED_DISK_SIZE_MAP[
      diskTypePrefix as keyof typeof AZURE_MANAGED_DISK_SIZE_MAP
    ];
...
function mapAzureManagedDiskSize(
  diskTypePrefix: string,
  diskName: string,
): number {
  const diskSizes =
...
function mapUltraDiskSize(requestedSizeGiB: number): number {
  ...
  return (
    Math.ceil(requestedSizeGiB / AZURE_ULTRA_DISK_SIZE_STEP) *
...
function getAwsReplicationGroupNodeCount(
  obj: Record<string, unknown> | undefined | null,
): number {
