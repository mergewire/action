...
describe('parseSlackWebhooks', () => {
  it('should parse a simple name=url pair', () => {
    ...
    expect(result.get('alerts')).toBe('https://hooks.slack.com/services/T1/B1/xxx')
  })
  ...
  it('should parse multiple channels', () => {
    ...
    expect(result.get('security')).toBe('https://hooks.slack.com/services/T2/B2/yyy')
  })
  ...
  it('should ignore comment lines starting with #', () => {
    const input = ['# This is a comment', 'alerts=https://hooks.slack.com/services/T1/B1/xxx'].join(
      '\n'
    )
    const result = parseSlackWebhooks(input)
    expect(result.size).toBe(1)
  ...
  it('should ignore lines with no = separator', () => {
    const input = ['not-a-valid-line', 'alerts=https://hooks.slack.com/services/T1/B1/xxx'].join(
      '\n'
    )
    const result = parseSlackWebhooks(input)
    expect(result.size).toBe(1)
  ...
  it('should handle URL with = in the value', () => {
    ...
    expect(result.get('alerts')).toBe('https://hooks.slack.com/services/T1/B1/xxx=extra')
  })
})
...
`.trim()
    ...
    const channels = await resolveSlackChannels(configPath, ['prod-destructive'])
...
`.trim()
    ...
    const channels = await resolveSlackChannels(configPath, ['rule-a', 'rule-b'])
...
`.trim()
  ...
  it('should return empty array when config file does not exist', async () => {
    const channels = await resolveSlackChannels('/nonexistent/.mergewire.yml', ['rule-a'])
...
`.trim()
    ...
    const webhookMap = new Map([['alerts', 'https://hooks.slack.com/services/T1/B1/xxx']])
    ...
    expect(vi.mocked(fetch).mock.calls[0]?.[0]).toBe('https://hooks.slack.com/services/T1/B1/xxx')
  })
...
`.trim()
    ...
    const webhookMap = new Map([['other-channel', 'https://hooks.slack.com/services/T1/B1/xxx']])
    await sendRuleSlackNotifications({
      webhookMap,
    ...
    expect(core.warning).toHaveBeenCalledWith(expect.stringContaining('missing-channel'))
  })
...
`.trim()
    ...
    expect(core.warning).toHaveBeenCalledWith(expect.stringContaining('alerts'))
  })
