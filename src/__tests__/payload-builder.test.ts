...
import { buildRoutingPayload, validateNoForbiddenFields } from '../payload-builder.js'
...
describe('buildRoutingPayload', () => {
  it('should build payload from terraform plan', () => {
    ...
    expect(payload.pricingResources![0].unpricedReason).toBe('missing_dimensions')
  })
  ...
  it('should NOT include forbidden fields in payload', () => {
    ...
    expect(payload.pricingResources).toHaveLength(1)
    const pricingResource = payload.pricingResources![0] as Record<string, unknown>
  ...
  it('should include pricing resources for supported families', () => {
    ...
    expect(azure.before).toEqual({ sku: 'Standard_B1s', osDiscriminator: 'linux' })
