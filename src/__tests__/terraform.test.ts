...
import { runTerraform, isTerraformInstalled, validateTerraform } from '../terraform.js'
...
describe('runTerraform', () => {
  ...
  it('should run terraform init, plan, and show', async () => {
    ...
    expect(mockExec).toHaveBeenCalledWith('terraform', ['init', '-no-color'], expect.any(Object))
    ...
    expect(mockExec).toHaveBeenCalledWith(
      'terraform',
      ['plan', '-out', expect.stringContaining('tfplan.bin'), '-no-color', '-input=false'],
      expect.any(Object)
    )
  ...
  it('should create workspace if it does not exist', async () => {
    ...
    mockExec.mockImplementation(async (cmd, args, options) => {
      ...
      if (callIndex === 2 && args?.includes('workspace') && args?.includes('select')) {
      ...
      if (cmd === 'terraform' && args?.includes('show')) {
        if (options?.listeners?.stdout) {
          options.listeners.stdout(Buffer.from(JSON.stringify({ resource_changes: [] })))
        }
      }
      return 0
    })
  ...
  it('should throw error if directory does not exist', async () => {
    ...
    await expect(runTerraform('./nonexistent')).rejects.toThrow('does not exist')
  })
