...
export function parseWebhooks(input: string): Map<string, string> {
  ...
  for (const line of input.split("\n")) {
    ...
    if (!url.startsWith("https://")) {
      core.warning(
        `webhooks: skipping "${name}" — URL must start with https://`,
      );
      continue;
    }
...
export async function resolveWebhooks(
  ...
  try {
    ...
    core.warning(
      "webhook-sender: could not parse .mergewire.yml, skipping webhook resolution",
    );
    return [];
  }
...
export async function sendRuleWebhooks(options: {
  ...
  if (hooks.length === 0) {
    core.info(
      "  No rules with notify.webhook found among matched rules — skipping",
    );
    return;
  }
  ...
  const promises = hooks.map(async (hookName) => {
    ...
    if (!webhookUrl) {
      core.warning(
        `  webhooks: no webhook URL configured for name "${hookName}" — skipping`,
      );
      return;
    }
    ...
    if (result.success) {
      ...
      core.warning(
        `  Failed to send webhook to "${hookName}": ${result.error}`,
      );
    }
  });
