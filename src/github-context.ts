...
export function extractGitHubContext(): GitHubContext {
  ...
  const baseRef =
    typeof pr.base === 'object' && pr.base !== null ? (pr.base as { ref?: string }).ref : undefined
  const headRef =
    typeof pr.head === 'object' && pr.head !== null ? (pr.head as { ref?: string }).ref : undefined
  const headSha =
    typeof pr.head === 'object' && pr.head !== null ? (pr.head as { sha?: string }).sha : undefined
