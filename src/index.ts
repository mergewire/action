...
import { parseSlackWebhooks, sendRuleSlackNotifications } from './slack-sender.js'
...
async function writeBackToGitHub(options: WriteBackOptions): Promise<void> {
  ...
  if (applyLabels) {
    try {
      await applySeverityLabel(octokit, owner, repo, pullNumber, evaluation.severity)
...
async function applySeverityLabel(
  ...
  const allSeverityLabelNames = Object.values(labelConfig).map((config) => config.name)
  ...
  for (const existingName of currentLabelNames) {
    if (allSeverityLabelNames.includes(existingName) && existingName !== name) {
      try {
        ...
        core.warning(`  Failed to remove conflicting label '${existingName}': ${error instanceof Error ? error.message : String(error)}`)
      }
    }
  }
